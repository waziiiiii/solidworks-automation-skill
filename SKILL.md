...
metadata: { "short-description": "AutoLife optimized SW CAD workflow", "openclaw": { "homepage": "https://github.com/waziiiiii/solidworks-automation-skill", "os": ["win32"], "requires": { "anyBins": ["python", "py"] } } }
---
...
**硬性规则**：每次新建模/改模任务的第一条实质回复必须先输出评分表；必须等待用户确认继续；未收到明确确认前，只能评分、解释评分依据、补充询问输入信息，不能开始制作模型。

评分时优先读取 `references/robot-part-complexity-scoring.md`；若上下文或时间有限，也必须至少按下表直接评分：
...
| 设计意图/参数化 | 15 |
| 装配关系/接口 | 15 |
| SolidWorks 自动化稳定性 | 15 |
| 受力/运动风险 | 10 |
| 精度/公差要求 | 10 |

等级区间：L1 = 0-30，Codex 可完全完成；L2 = 31-55，Codex 可完成但需要人工确认；L3 = 56-75，Codex 只做辅助；L4 = 76-100，不建议 Codex 承担主体设计。复杂自由曲面、A 面外观、人机曲面或高度拟真外壳若无法通过稳定 API 生成，直接 L4；若需要大量人工调面，最低 L3。
...
请确认是否按该等级继续建模。确认后我再开始 SolidWorks 自检和建模。
```

流程：

1. 根据用户提供的文字、图片、草图、旧模型、尺寸、装配约束和用途，对目标模型按 8 个维度评分。
2. 向用户汇报总分、等级、逐项得分、主要理由、Codex 可承担内容、需要人工确认内容、工程风险标签，以及是否触发一票否决。
3. 明确询问用户是否确认按该等级继续建模。
4. 只有用户明确确认继续后，才进入 SolidWorks 自检和正式建模流程。
